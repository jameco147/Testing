...
class Player{
    ...
    static fromDatasetJSONObject(playerId,jsonObject){
        ...
        //si value no es un número lo lo tranformamos a un entero
        if (isNaN(player.value)){
            var strEx = player.value;
            //primer paso: fuera puntos
            strEx = strEx.replace(".","");
            //segundo paso: fuera €
            strEx = strEx.replace("€","");
            //cambiamos la M por 00000
            strEx = strEx.replace("M","00000");
            //Convertimos el número a un entero
            player.value = parseFloat(strEx);
            }
